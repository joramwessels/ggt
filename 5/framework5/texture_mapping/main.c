...
DrawGLScene(void)
{
    ...
    for (int t = 0; t < 12; t++)
    {
        ...
        // Draw up to twice the amount of leafs randomly, distributed
        // equally around the stem, with each a small random offset.
        for (int l=0; l<(leafs + (int)(rand_float() * leafs)); l++) {
            glPushMatrix();
            glRotatef(((360.0/leafs) * l) + (rand_float() * 50), 0, 1, 0);
